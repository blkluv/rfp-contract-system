...
const EditResponsePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchResponse = async () => {
      try {
        const responseData = await apiService.getResponseById(responseId);
        const responseInfo = responseData.response;
        setResponse(responseInfo);
        
        // Fetch RFP details
        const rfpData = await apiService.getRFPById(responseInfo.rfpId);
        setRfp(rfpData.rfp);
        
        // Reset form with response data
        reset({
          proposal: responseInfo.proposal || '',
          proposedBudget: responseInfo.proposedBudget || null,
          timeline: responseInfo.timeline || '',
          notes: responseInfo.notes || '',
        });
      } catch (err: any) {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      await apiService.updateResponse(responseId, data);
  ...
  if (user?.role !== 'supplier' || response.supplierId !== user?.id) {
    return (
      <Layout>
        <Container maxWidth="lg">
          <Alert severity="error">
                ...
                <Grid container spacing={3}>
                  ...
                  <Grid item xs={12}>
                    <Box display="flex" gap={2}>
                      ...
                      >
                        {isSubmitting ? <CircularProgress size={24} /> : 'Update Response'}
                      </Button>
                      <Button
                        variant="outlined"
